...
export default function GrammarPanel({ grammar, activeGrammarId }) {
  ...
  if (grammar.length === 0) {
    return <p className="p-4 text-sm text-[#8c7761]">No grammar yet.</p>;
  }
  return (
    <div className="divide-y divide-[#ede3d5]">
      {grammar.map((entry) => (
        ...
        >
          <p className="font-serif-jp text-lg font-semibold text-[#9a4a3a]">
            {entry.pattern}
          </p>
          <p className="mt-1 text-sm leading-6 text-[#3a3027]">
            {entry.explanation}
          </p>
          <p className="mt-1 text-xs leading-5 text-[#6b5d4f]">
            {entry.example}
          </p>
        </div>
      ))}
