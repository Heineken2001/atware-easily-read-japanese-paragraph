export default function VocabPanel({ vocabulary, activeWordId }) {
  if (vocabulary.length === 0) {
    return <p className="p-4 text-sm text-[#8c7761]">No vocabulary yet.</p>;
  }
  return (
      ...
      {vocabulary.map((entry) => (
        ...
        >
          <div className="flex items-baseline gap-2">
            <span className="font-serif-jp text-lg font-semibold text-[#2d241c]">
              {entry.word}
            </span>
            <span className="text-sm text-[#8c7761]">{entry.reading}</span>
          </div>
          <p className="mt-1 text-sm font-medium text-[#26435f]">
            {entry.meaning}
          </p>
          <div className="mt-2 space-y-1">
            {entry.examples.map((ex, i) => (
              <p key={i} className="text-xs leading-5 text-[#6b5d4f]">
                {ex}
              </p>
            ))}
          </div>
