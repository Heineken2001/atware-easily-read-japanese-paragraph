...
describe("parseResponse", () => {
  ...
  it("throws when translation is missing", () => {
    expect(() => parseResponse({ ...valid, translation: undefined })).toThrow(
      "Invalid response format",
    );
  });

  it("throws when tokens is missing", () => {
    expect(() => parseResponse({ ...valid, tokens: undefined })).toThrow(
      "Invalid response format",
    );
  });

  it("throws when vocabulary is missing", () => {
    expect(() => parseResponse({ ...valid, vocabulary: undefined })).toThrow(
      "Invalid response format",
    );
  });

  it("throws when grammar is missing", () => {
    expect(() => parseResponse({ ...valid, grammar: undefined })).toThrow(
      "Invalid response format",
    );
  });

  it("throws when tokens is not an array", () => {
    expect(() => parseResponse({ ...valid, tokens: "bad" })).toThrow(
      "Invalid response format",
    );
  });
  ...
  it("throws when a token is missing text", () => {
    expect(() =>
      parseResponse({
        ...valid,
        tokens: [{ reading: null, wordId: null, grammarId: null }],
      }),
